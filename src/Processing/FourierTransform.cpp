...
void FourierTransform::setComplex(float* r, int rLength, float* i, int iLength)
{
	...
	{
		printf("FourierTransform.setComplex: the two arrays must be the same length as their member counterparts");
	}
	else
	{
...
void FourierTransform::linAverages(int numAvg)
{
	...
	{
		printf("The number of averages for this transform can be at most %d.", spectrumLength / 2);
	}
	else
	{
		if (averages != nullptr) delete averages;
...
void FourierTransform::logAverages(int minBandwidth, int bandsPerOctave)
{
	...
	printf("Number of octaves: %d", octaves);
...
int FourierTransform::freqToIndex(float freq)
{
	...
	int i = static_cast<int>(round(timeSize * fraction));
	return i;
}
...
void FourierTransform::forward(float* buffer, int bufferLength, int startAt)
{
	...
	{
		printf("FourierTransform.forward: not enough samples in the buffer between %d and %d to perform a transform.", startAt, bufferLength);
		return;
	}
