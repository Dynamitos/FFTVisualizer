...
std::unique_ptr<AudioInputContainer> AudioFileReader::readSamples()
{
    ...
    if (sampleContainer->allocatedChannels < frame->channels)
    {
        if (sampleContainer->samples != nullptr)
        {
            ...
            for (int32_t i = 0; i < sampleContainer->allocatedChannels; ++i)
            {
                //delete channel data
    ...
    if(frame->linesize[0] > audioByteSize)
    {
        for(int32_t i = 0; i < frame->channels; ++i)
        {
            //planar audio
            std::memcpy(sampleContainer->samples[i], &frame->extended_data[0][i*audioByteSize], audioByteSize);
        }
    }
    else
    {
    ...
    sampleContainer->timeStamp = lastTimeStamp;
    lastTimeStamp += (frame->nb_samples * 1000000000ull) / codecContext->sample_rate;
    //printf("Timestamp: %lld\n", sampleContainer->timeStamp);
