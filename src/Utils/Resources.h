...
#define GL_CHECK() { GLenum error = glGetError(); assert(error == GL_NO_ERROR);}
struct Line
{
	glm::vec2 start;
	glm::vec2 end;
...
struct AudioInputContainer
{
		...
		, timeStamp{ copy.timeStamp }
	{
		...
		for (int32_t i = 0; i < copy.allocatedChannels; ++i)
		{
	...
	AudioInputContainer& operator=(const AudioInputContainer& other)
	{
		if (this != &other)
		{
			if (samples != nullptr)
			{
				for (int32_t i = 0; i < allocatedChannels; ++i)
				{
					delete[] samples[i];
				}
				delete[] samples;
			...
			for (int32_t i = 0; i < other.allocatedChannels; ++i)
			{
