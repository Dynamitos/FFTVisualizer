...
RawModel* Loader::loadToVAO(float* vertices, uint32_t vertexSize, float* texCoords, uint64_t texCoordSize)
{
...
RawModel* Loader::loadToVAO(float* vertices, uint32_t vertexSize, float* texCoords, uint64_t texCoordSize, float* normals, uint64_t normalSize, uint16_t* indices, uint32_t indexSize, const char* name)
{
	GLuint vao = createVAO();
...
RawModel* Loader::loadToVAO(float* vertices, uint32_t vertexSize, int dimensions)
{
	GLuint vao = createVAO();
...
GLuint Loader::loadTexture(const char* fileName)
{
	...
	int x, y, channels;
	std::string textureBasePath = "./textures/";
