...
void FFT::scaleBand(int i, float s)
{
	if (s < 0)
	{
		std::cerr << "Can't scale a frequency band by a negative value." << std::endl;
		return;
	}
...
void FFT::setBand(int i, float a)
{
	if (a < 0)
	{
		std::cerr << "Can't set a frequency band to a negative value." << std::endl;
	}
...
void FFT::forward(float * buffer, int bufferLength)
{
	...
	{
		std::cerr << "FFT.forward: The length of the passed sample buffer must be equal to timeSize()." << std::endl;
	}
...
void FFT::forward(float * buffReal, int rLength, float * buffImag, int iLength)
{
	...
	{
		std::cerr << "FFT.forward: The length of the passed buffers must be equal to timeSize()." << std::endl;
		return;
	}
