...
void SampleConverter::convertFrame(AVFrame* inFrame, std::vector<uint8_t>& outputBuffer, int& dataSize)
{
	uint8_t* outputPointer = nullptr;
	int maxOutput = swr_get_out_samples(swrContext, inFrame->nb_samples);
	if (outputBuffer.size() < maxOutput)
	{
		outputBuffer.resize(static_cast<uint64_t>(maxOutput) * 4);
	}
	outputPointer = outputBuffer.data();
	int ret = swr_convert(swrContext, &outputPointer, inFrame->nb_samples, (const uint8_t * *)inFrame->extended_data, inFrame->nb_samples);
	...
	dataSize = av_samples_get_buffer_size(nullptr, initInfo.inChannelCount, inFrame->nb_samples, initInfo.inSampleFormat, 1);
}
